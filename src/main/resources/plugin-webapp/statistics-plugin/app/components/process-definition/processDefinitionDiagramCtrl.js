'use strict'
ngDefine('cockpit.plugin.statistics-plugin.components', function(module) {
	...
	module.controller('processDefinitionDiagramCtrl', ['$scope', 'DataFactory', 'SettingsFactory', '$modal', '$filter', function($scope, DataFactory, SettingsFactory, $modal, $filter){
			...
			then(function() {
				...
				if(len == 0) {
					$scope.modalInstance = $modal.open({
						templateUrl: 'noHistoryModalView',
						controller: 'noHistoryModalController',
						size: 'sm',
						resolve: {
					...
					$scope.modalInstance = $modal.open({
						templateUrl: 'activityHistoryModalView',
						controller: 'activityHistoryModalController',
						size: 'lg',
						resolve: {
							activityId: function () {
								return id;
