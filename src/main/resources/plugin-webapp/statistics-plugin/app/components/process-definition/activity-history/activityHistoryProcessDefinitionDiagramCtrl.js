...
ngDefine('cockpit.plugin.statistics-plugin.activity-history', function(module) {
	...
	module.controller('activityHistoryProcessDefinitionDiagramCtrl', ['$scope', 'DataFactory', 'SettingsFactory', '$modal', '$filter', function($scope, DataFactory, SettingsFactory, $modal, $filter){
			...
			then(function() {
				...
				if(len == 0) {
					...
					$scope.modalInstance = $modal.open({
						...
						controller: 'activityHistoryModalCtrl',
						size: 'lg',
						resolve: {
							activityId: function () {
								return id;
